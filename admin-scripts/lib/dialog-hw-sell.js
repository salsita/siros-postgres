...
const questions = {
  [dialogStates.init]: {
    text: '[main] enter hw id to sell, (s)how hw, or (q)uit (<number>/S/q)',
    handlers: [
      {
        match: /^\d+$/,
        code: async (context, answer) => {
          ...
          questions[dialogStates.sellHwDate].text = `[sell hw] enter effective date (in YYYY-MM-DD format, press enter for "${context.sellHw.date}")`;
          return dialogStates.sellHwUserId;
        },
      },
      {
        match: /^[s]{0,1}$/i,
  ...
  [dialogStates.sellHwUserId]: {
    text: '[sell hw] enter new user (owner) id, (l)ist existing users, or go (b)ack (<number>/L/b)',
    handlers: [
      {
        match: /^\d+$/,
        code: async (context, answer) => {
          const id = parseInt(answer, 10);
          context.sellHw.userId = id;
          ...
          context.sellHw.user = user;
          return dialogStates.sellHwDate;
        },
      },
      {
        match: /^[l]{0,1}$/i,
  ...
  [dialogStates.sellHwDate]: {
    text: '<replaced from [dialogStates.init], handler (number)>',
    handlers: [
      {
        match: /^$/,
        code: (context) => {
          const { hw } = context.sellHw;
          /* eslint-disable max-len */
          let price = config.hwItems.getAgedPrice(hw.purchase_price, hw.purchase_date, context.sellHw.date);
          if (hw.max_price !== null) { price = Math.min(hw.max_price, price); }
          if (hw.condition === 'new') { price = hw.purchase_price; }
          context.sellHw.price = price;
          questions[dialogStates.sellHwPrice].text = `[sell hw] enter effective price (number, press enter for (${context.sellHw.price}))`;
          return dialogStates.sellHwPrice;
        },
      },
      {
        match: /^20\d{2}-\d{2}-\d{2}$/,
        code: (context, answer) => {
          context.sellHw.date = answer;
          const { hw } = context.sellHw;
          let price = config.hwItems.getAgedPrice(hw.purchase_price, hw.purchase_date, context.sellHw.date);
          if (hw.max_price !== null) { price = Math.min(hw.max_price, price); }
          if (hw.condition === 'new') { price = hw.purchase_price; }
  ...
  [dialogStates.sellHwPrice]: {
    text: '<replaced from [dialogStates.sellHwDate], all handlers>',
    handlers: [
      {
        match: /^$/,
        code: (context) => {
          context.sellHw.comment = `Sold to ${context.sellHw.user.name} on ${context.sellHw.date} for ${context.sellHw.price}.`;
          questions[dialogStates.sellHwComment].text = `[sell hw] update hw item comment to "${context.sellHw.comment}"? (Y/n)`;
          return dialogStates.sellHwComment;
        },
      },
      {
        match: /^\d+$/,
        code: (context, answer) => {
          context.sellHw.price = parseInt(answer, 10);
          context.sellHw.comment = `Sold to ${context.sellHw.user.name} on ${context.sellHw.date} for ${context.sellHw.price}.`;
          questions[dialogStates.sellHwComment].text = `[sell hw] update hw item comment to "${context.sellHw.comment}"? (Y/n)`;
          return dialogStates.sellHwComment;
        },
  ...
  [dialogStates.sellHwComment]: {
    text: '<replaced from [dialogStates.sellHwPrice], all handlers>',
    handlers: [
      {
        match: /^[y]{0,1}$/i,
        code: (context) => {
  ...
  [dialogStates.sellHw]: {
    text: '[sell hw] do you want to proceed? (y/N)',
    handlers: [
      {
        match: /^[y]$/i,
        code: async (context) => {
          const id = await context.dbQuery.sellHw({
            id: context.sellHw.id,
            oldUser: context.sellHw.hw.user_id,
            newUser: context.sellHw.userId,
            date: context.sellHw.date,
          ...
          process.stdout.write(`\nhw item with id (${id}) successfully sold\n\n`);
          return dialogStates.init;
        },
      },
      {
